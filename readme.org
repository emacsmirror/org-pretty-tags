...
- There is a convenience function to change =org-pretty-tags-mode= in all
  Org mode buffers.
...
**** Activate/Deactivate For an Org Mode and org Agenda Buffers
...
- If the tag surrogates don't show up as expected then try "reset"
  with {C-u M-x org-pretty-tags-mode RET}. Note the C-u.  Or turn
